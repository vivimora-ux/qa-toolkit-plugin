...
or an earlier `/rs-aut`, `/pr-explainer`, `/test-plan`, or `/test-suite`
invocation this
session. See
...
has been set yet, give a short, balanced pass — structure first, then a
brief trace, then risk — rather than asking a clarifying question.
...
- **Required header block** — at the top of the file, before any other
  ...
  - A "Commands used" section listing the full command transcript so
    ...
    verbatim, including whatever modifier and topic arguments were
    given. Render each as inline code, e.g.
    `` `/rs-aut senior risk the payments service` ``. Append to this
    list (don't rewrite past entries) as more invocations happen later
    in the day.
